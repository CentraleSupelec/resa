...
function filterRoom(filters, room) {
  ...
  if (
    Array.isArray(displayCampuses) &&
    displayCampuses.length > 0 &&
    (!Array.isArray(displayBuildings) || displayBuildings.length === 0)
  ) {
  ...
  if (
    ...
    (!Array.isArray(displayCampuses) || displayCampuses.length === 0)
  ) {
  ...
  if (
    ...
    if (
      !displayCampuses.includes(room.campus) &&
      !displayBuildings.includes(room.building)
    )
      return false;
  }
